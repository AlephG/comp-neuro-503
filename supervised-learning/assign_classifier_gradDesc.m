...
function [loss_sum, weights, trial_loss] = assign_classifier_gradDesc(lr, alpha, nTrials) 
...
for iTrial=1:nTrials    % loop over trials (iterations)
   ...
   for iDataPt=1:nDataPts
        ...
        loss = E^2;     % loss = error-squared SHOULD THIS INCLUDE WEIGHT DECAY??
...
%    set(h,'YData',x2);       % "h" from above, "h=plot(...)"
   ...
   %Return values for weigths
   weights(1, iTrial) = w(1);
   weights(2, iTrial) = w(2);
   weights(3, iTrial) = w0;
   
   %Return values for loss
   trial_loss(iTrial) = loss_sum; %MSE
